...
function getNextFriday(date) {
  const curDt = new Date(date);
  const currDay = curDt.getDay();
  const toFr = (5 - currDay + 7) % 7 || 7;
  curDt.setDate(curDt.getDate() + toFr);
  return curDt;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  return Math.ceil(
    (1 + Date.parse(dateEnd) - Date.parse(dateStart)) / (864 * 100000)
  );
}
...
function isDateInPeriod(date, period) {
  let dInP = false;
  const dInMs = new Date(date).valueOf();
  const pStInMs = new Date(period.start).valueOf();
  const pEndInMs = new Date(period.end).valueOf();

  if (dInMs >= pStInMs && dInMs <= pEndInMs) {
    dInP = true;
    return true;
  }
...
function formatDate(date) {
  ...
  let hh = nD.getUTCHours();
  let mm = nD.getUTCMinutes();
  let ss = nD.getUTCSeconds();
  const prd = hh >= 12 ? 'PM' : 'AM';
  hh %= 12;
  hh = hh || 12;
  mm = mm < 10 ? `0${mm}` : mm;
  ss = ss < 10 ? `0${ss}` : ss;

  return `${nD.toLocaleDateString('en-US', { timeZone: 'UTC' })}, ${hh}:${mm}:${ss} ${prd}`;
}
...
function getCountWeekendsInMonth(month, year) {
  let wwe = 0;
  const tD = new Date(year, month, 0).getDate();
  for (let d = 1; d <= tD; d += 1) {
    const dt = new Date(year, month - 1, d);
    const dOfW = dt.getDay();
    if (dOfW === 0 || dOfW === 6) wwe += 1;
  }
  return wwe;
}
...
function getWeekNumberByDate(date) {
  ...
  const stYD = new Date(y, 0, 1);
  const lD = new Date(date);
  const stYDInMs = Date.parse(stYD);
  const lDInMs = Date.parse(lD);
  const d = (lDInMs - stYDInMs) / 86400000 + 1;
  let r = 0;
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const r = [];
  const cD = new Date(period.start.split('-').reverse().join('-'));
  const eD = new Date(period.end.split('-').reverse().join('-'));

  while (cD <= eD) {
    for (let i = 0; i < countWorkDays && cD <= eD; i += 1) {
      r.push(cD.toISOString().split('T')[0].split('-').reverse().join('-'));
